...
export default function ShowAll({ DummyData }) {
    ...
    const mapDummyData = DummyData.teams.map((data, index) => {
        return (
            <>
                {data.is_archived ? <div className="w-4/5 h-fit border m-10 rounded-md bg-[#E4E7EC]" key={index}>
                    ...
                    <div className="w-4/5 h-fit border m-10 rounded-md" key={index}>
                        <div className="flex flex-row" >
                            <div className="w-fit flex flex-row mt-5 ">
                                <img src={data.image} className="w-1/5 ml-4" />
                                <div className="flex flex-col">
                            ...
                            <div className="w-1/2 flex flex-row justify-end mt-5 mr-4">
                                {data.is_favorited ? <Icon icon="entypo:star" color="gold" width="30" /> : <Icon icon="entypo:star-outlined" width="30" className="opacity-30" />}
                            </div>
                        </div>
                        <p className="pl-4 pb-5 pt-5">{data.description}</p>
                        <div className="border-t flex flex-row ">
